...
app.get("/api/compliments", async (req, res) => {
  // TODO: Get a list of messages sent from a specific number --done
  const sentMessages = await client.messages.list({ from: twilioPhoneNumber }); //[];
  // TODO: Gather only the body of those messages for sending to the client  --done
  const compliments = sentMessages.map(message => message.body); //get all messages, return array of message body
...
app.post("/api/compliments", async (req, res) => {
  ...
  // TODO: Send a message  --done  --CAtch any error messages
  try {
